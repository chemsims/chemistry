...
extension QuizQuestion {
    static let reactionComparisonQuizQuestions = [
        ...
        QuizQuestion(
            question: """
            A student conducts an experiment that consists on making $10 mL$ of a $0.20 M$ violet solution \
            react with $10 mL$ of a $0.20 M$ base solution. After this, the student takes a sample of the \
            resulting solution and places it in a UV-Vis spectrophotometer, records the absorbance and plots \
            some graphs. What is the order of the reaction?
            """,
        ...
        QuizQuestion(
            ...
            explanation: """
            For the rate law equation: Rate = k[A]^3^[B], knowing that concentrations are in units of M, and \
            Rate in M/s, you can solve for k: $M/s = k(M)^3^(M)$ →➝ $M/s = kM^4^$ ➝ $k = M/(M^4^)(s)$ ➝ \
            $k = 1/(M^3^)(s)$ or what is the same s^-1^M^-3^. A short way to know the units for the rate \
            constant are using M^(1-n)^/s where n is the overall order of the equation. When $n = 4$, \
            $M^(1-4)^/s = s^-1^M^-3^$.
            """,
            difficulty: .easy
        ),
        QuizQuestion(
            ...
            $x = 2$; the reaction is second order for [A]. When [B] goes from 0.230 to 0.460 (it goes up by \
            a factor of 2) while A remains constant, the Rate remains constant. If the rate doesn't vary \
            with the concentration of B, it means the reaction is zero order for [B]. Writing the complete \
        ...
        QuizQuestion(
            ...
            explanation: """
            The rate-determining step would probably be Rate = k[F]^2^[G]. Of all the rate laws, this one is \
            of the highest order (third overall) and implies that a successful collision of 3 molecules has \
            to take place for the reaction to occur. Since the probabilities of this happening are lower, \
            the chances of this reaction to be slower than the other ones is greater. Being the slowest of \
        ...
        QuizQuestion(
            question: """
            The rate constant for a second order reaction $(A ➝ B)$ is $0.11 M^-1^s^-1^$. How much of A \
            would be remaining after 12 seconds has passed? Considering the initial concentration of A is \
            $0.94 M$?
            """,
        ...
        QuizQuestion(
            ...
            where $x = 1$; the reaction is first order for [Br_2_]. When [Br_2_] drops to 0.0066, if [NO] \
            were maintained at 0.0176, then we could determine that the Rate would have been 2.15x10^-4^ \
            (half of 4.30x10^-4^) since we know it's of first order for [Br_2_]. In order for the Rate to be \
            8.60x10^-4^ when [NO] goes from 0.0176 to 0.0352 (it's doubled, goes up by a factor of 2), it \
            had to be quadrupled (go up by a factor of 4). $2^x^ = 4$. where $x = 2$; the reaction is of \
            second order for [NO].
            """,
            difficulty: .medium,
            table: QuizTable(
        ...
        QuizQuestion(
            ...
            $4.30x10^-4^ = k(0.0176^2^)(0.0132)$. Clearing for k: \
        ...
        QuizQuestion(
            ...
            where $x = 2$; the reaction is second order for [Cl^-^]. When [MnO4^-^] goes from 0.0096 to \
            0.0048 (it's halved, goes up down by a factor of 2) the Rate goes from 3.51x10^-12^ to \
            1.76x10^-12^ (it's halved, it goes down by a factor of 2). $2^x^ = 2$ where $x = 1$; the \
            reaction is first order for [MnO_4_^-^].
            """,
            difficulty: .medium,
            table: QuizTable(
                rows: [
        ...
        QuizQuestion(
            ...
            $Rate = k[A][B]$. Furthermore, the slowest step, in this case Step 2, is also called the \
            rate-determining step, because the overall reaction cannot go faster than the slowest of the \
            steps. In other words, the rate for this reaction would be $Rate = k[I][A]$ but since I is only \
            the intermediate, then the actual rate law is: $Rate = k[A]^2^[B]$.
            """,
            difficulty: .hard
        ),
        QuizQuestion(
        ...
        QuizQuestion(
            question: """
            The half-life of a first order reaction (A ➝ B) is equal to 198 seconds. What percentage of the \
            reactant would be left after 390 seconds have passed?
            """,
            correctAnswer: "25.5%",
            otherAnswers: [
                "12%",
            ...
            $k = ln(2)/198 = 0.0035 seconds^-1^$.The equation for a first order reaction is \
            $ln[A] = ln[A_0_] - kt$. After 390 seconds have passed, there's a fraction of A remaining, or: \
            [4A] = X[A_0_]$. Replacing we get: $ln(X[A_0_]) = ln[A_0_] - (0.0035)(390)$. Moving all \
            logarithmic expressions to one side: $(0.0035)(390) = ln([A_0_]) - lnX[A_0_]$. Using logarithmic \
            properties: $(0.0035)(390) = ln([A_0_]/X[A_0_])$. Cancelling [A_0_] from the expression and \
            solving numerically: $(1.365) = ln(1/X)$. Applying exponential to both sides: $e^1.365^ = 1/X$. \
        ...
        QuizQuestion(
            question: """
            The half-life of a first order reaction (A ➝ B) is equal to 232 seconds. What percentage of the \
            reactant would be left after 490 seconds have passed?
            """,
            correctAnswer: "23.1%",
            otherAnswers: [
                "15.5%",
            ...
            $k = ln(2)/232 = 0.00298 seconds^-1^$. The equation for a first order reaction is \
            $ln[A] = ln[A_0_] - kt$. After 490 seconds have passed, there's a fraction of A remaining, or: \
            $[A] = X[A_0_]$. Replacing we get: $ln(X[A_0_]) = ln[A_0_] - (0.00298)(490)$. Moving all \
            logarithmic expressions to one side: $(0.00298)(490) = ln([A_0_]) - lnX[A_0_]$. Using \
            logarithmic properties: $(0.00298)(490) = ln([A_0_]/X[A_o_])$. Cancelling [A_0_] from the \
            expression and solving numerically: $(1.464) = ln(1/X)$. Applying exponential to both sides: \
            $e^1.464^ = 1/X$. Finally solving for X: $X = 1/e^1.464^ = 0.231$ which is the same as 23.1%.
            """,
            difficulty: .easy
        ),
        QuizQuestion(
            ...
            """,
            correctAnswer: """
            The rate constant for the decay of I-131 is greater to the rate constant for the decay of ruthenium-106.
            """,
            otherAnswers: [
                """
            ...
            get: $k = ln(2) / t_1/2_$. The higher the half-life is, the smaller the rate constant k is. For \
            ruthenium-106, its massively big half-life of 1 year is much greater than the 8 days half-life \
            of I-131 decay, making the ruthenium-106 rate constant much smaller.
            """,
            difficulty: .medium
        ),
    ]
